...
export async function POST(req) {
  ...
  const rateLimitKey = uid ? `user_${uid}` : `ip_${getClientIp(req)}`;
  const rateLimit = await checkRateLimit(rateLimitKey);
  if (!rateLimit.allowed) {
    return NextResponse.json(
      { error: `You've hit the scan limit for now — try again in about ${rateLimit.retryAfterMin} minute${rateLimit.retryAfterMin === 1 ? '' : 's'}.` },
      { status: 429 }
    );
  }
